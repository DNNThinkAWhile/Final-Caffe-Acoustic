...
if [ ! -f Lexicon.fst ]; then
   ...
   grep -v -E "^;" timitdic.txt | gsed -e 's/\///g' -e 's/[0-9]//g' -e 's/\~[a-zA-Z_\~]* / /g' > lexicon.txt
...
# generate input.fst
#   ./timit_norm_trans.pl -i $input -m phones.60-48-39.map -from 60 -to 39 | gsed -e 's/\bsil\b/ /g'| gsed -e 's/  / /g' > $tmpdir/input.39 
#   
#   # read example
#   j=0; 
...
#   echo "$j 0" >> $tmpdir/input.log
    ...
    do
#echo $p
        ./generate $p phones.60-39.map rule.out > $tmpdir/input.log
...
command+=" | gsed -e \"s:':COMMA:g\" | gsed "
command+=$(while read phone; do echo " -e 's/\b${phone}\b/ /g'"; done < phone_list)
command+=" | gsed -e \"s:COMMA:':g\" "
command+=" | tr -s ' ' | gsed -e 's/^ //g' | sort | uniq"
